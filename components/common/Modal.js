...
const Modal = ({
  ...
  return createPortal(
    ...
    >
      <div
        className={cn(
          `${styling.components.modal} modal-box max-h-[calc(100vh-10rem)] p-0 pb-4 sm:pb-0 ${styling.flex.col} overflow-hidden`,
          boxClassName,
        )}
      >
        {title && (
        ...
        <div
          className={cn(
            `flex-1 overflow-y-auto p-4 sm:p-6 pt-2`,
            contentClassName,
          )}
        >
      ...
      <div
        ...
        onClick={onClose}
      >
        <button className="cursor-default w-full h-full outline-none">
          close
        </button>
      </div>
    </ThemeWrapper>,
