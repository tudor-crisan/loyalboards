  ...
  async (req, { session }) => {
    ...
    if (!body.title) {
      return responseError(
        titleRequired.message,
        titleRequired.inputErrors,
        titleRequired.status,
      );
    }
    ...
    if (!body.description) {
      return responseError(
        ...
        descriptionRequired.inputErrors,
        descriptionRequired.status,
      );
    }
...
export const DELETE = withApiHandler(
  async (req, { user }) => {
    const { postIdRequired, postNotFound, deleteSuccesfully } =
      settings.forms[TYPE].backend.responses;
